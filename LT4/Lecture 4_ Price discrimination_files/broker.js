var SR_url = window.location.toString().toLowerCase(); var _sr_config = "broker-config.js";if(/(www|js|i3)\.microsoft\.com\/library\/svy\/min\/(int_cle)\.htm/i.test(SR_url)){ _sr_config = "broker-config_cle.js"; }
...
}if(window.ActiveXObject){}},listen:function(){
	var ddckVal = COMSCORE.SiteRecruit.Utils.UserPersistence.getCookieValue('msresearch');if (/DDINPROGRESS/i.test(ddckVal)){COMSCORE.SiteRecruit.Broker.config={cookie:{name:"msresearch",path:"/",domain:".microsoft.com",duration:90,rapidDuration:0,expireDate:""}};COMSCORE.SiteRecruit.Broker.processDDInProgress();}var dom = document.domain.split('.');var ref='';dom=dom[dom.length-2]+"\."+dom[dom.length-1];var domRE=new RegExp(dom,"i");if (document.referrer!=""){ref=document.referrer;}if (ref !=""&&ref!=undefined&&!(domRE.test(ref))){COMSCORE.SiteRecruit.Broker.delayConfig=true;}else {COMSCORE.SiteRecruit.Broker.delayConfig=false;}
